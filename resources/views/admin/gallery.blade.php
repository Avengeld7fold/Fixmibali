<x-app-layout>
                    ...
                    <div class="grid grid-cols-1 gap-4 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5">
                        ...
                        @foreach ($images as $image)
                            ...
                            <div class="group relative rounded-2xl bg-white p-3 shadow-sm ring-1 ring-black/5">
                                ...
                                <form
                                    method="POST"
                                    action="{{ route('admin.gallery.destroy', ['filename' => $image['filename']]) }}"
    ...
    <x-modal name="gallery-upload" :show="false" maxWidth="2xl">
        <div class="admin-upload-modal" x-data="adminGalleryUpload()" x-init="init()">
            <div class="admin-upload-header">
                <div class="admin-upload-title">Upload file</div>
                <button
                    type="button"
            ...
            <div class="admin-upload-body">
                ...
                >
                    ...
                    <div class="admin-dropzone-text">
                        Drag and Drop file here or <span class="admin-dropzone-link">Choose file</span>
                    </div>
                    <input
                        type="file"
                        x-ref="fileInput"
        ...
        x-on:gallery-preview-open.window="url = $event.detail.url || ''; alt = $event.detail.alt || ''; show = true"
    ...
    <script>
        function adminGalleryUpload() {
            return {
                ...
                queue: [],
                init() {
                    const base = (window.location.pathname || '/dashboard/gallery').replace(/\/$/, '');
                ...
                cleanupAndClose() {
                    ...
                    if (tokens.length === 0) {
                        window.dispatchEvent(new CustomEvent('close-modal', { detail: 'gallery-upload' }));
                        this.queue = [];
                        this.errorMessage = '';
                        return;
                    }
        ...
        <script>
        (function () {
            const items = document.querySelectorAll('[data-gallery-preview]');
            items.forEach((item) => {
                item.addEventListener('click', () => {
                    const url = item.getAttribute('data-url') || '';
                    const alt = item.getAttribute('data-alt') || '';
                    window.dispatchEvent(new CustomEvent('gallery-preview-open', { detail: { url, alt } }));
                });
            });
        })();
    </script>
