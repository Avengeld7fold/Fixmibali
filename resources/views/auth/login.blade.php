<x-admin-auth-layout>
    <div class="w-full max-w-[420px]">
        <div class="auth-card auth-card-anim relative rounded-[32px] px-7 py-9 sm:px-9 sm:py-10">
            <div class="pointer-events-none absolute inset-x-6 top-6 h-16 rounded-3xl bg-white/70 blur-2xl"></div>

            <div class="relative">
                <div class="mx-auto flex h-14 w-14 items-center justify-center rounded-2xl border border-white/80 bg-white shadow-[0_12px_25px_rgba(15,23,42,0.12)] auth-float">
                    <img src="{{ asset('assets/img/favinco.png') }}" alt="Fixmi" class="h-7 w-7 object-contain" />
                </div>

                <div class="mt-6 text-center">
                    <h1 class="auth-title text-lg font-semibold text-slate-900 auth-fade-up auth-delay-1">Masuk dengan email</h1>
                    <p class="auth-subtitle mt-2 max-w-xs text-sm text-slate-500 auth-fade-up auth-delay-2 mx-auto">Masuk untuk mengelola website Fixmi</p>
                </div>

                <x-auth-session-status class="mt-5 text-sm text-slate-600 auth-fade-up auth-delay-3" :status="session('status')" />
                ...
                <form method="POST" action="{{ route('login') }}" class="mt-6 space-y-4 auth-fade-up auth-delay-4">
                    ...
                    <div>
                        ...
                        <div class="relative">
                            ...
                            <input id="login" class="auth-input w-full rounded-2xl px-11 py-3 text-sm text-slate-700 placeholder:text-slate-400 focus:border-sky-200 focus:outline-none focus:ring-2 focus:ring-sky-200/80" type="text" name="login" value="{{ old('login') }}" required autofocus autocomplete="username" placeholder="Email or Username" />
                        </div>
                    ...
                    <div>
                        ...
                        <div class="relative">
                            ...
                            <input id="password" class="auth-input w-full rounded-2xl px-11 py-3 pr-12 text-sm text-slate-700 placeholder:text-slate-400 focus:border-sky-200 focus:outline-none focus:ring-2 focus:ring-sky-200/80" type="password" name="password" required autocomplete="current-password" placeholder="Password" />
                            <button type="button" class="auth-eye-toggle absolute right-3 top-1/2 -translate-y-1/2 text-slate-400 transition hover:text-slate-600" data-password-toggle="password" aria-label="Tampilkan password" aria-pressed="false">
                    ...
                    @if (Route::has('password.request'))
                        <div class="flex justify-end">
                            <a class="text-xs font-semibold text-slate-500 transition hover:text-slate-700" href="{{ route('password.request') }}">
                                {{ __('Forgot password?') }}
                            </a>
                        </div>
                    @endif
